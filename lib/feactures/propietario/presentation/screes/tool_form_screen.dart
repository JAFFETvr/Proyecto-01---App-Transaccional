...
class ToolFormScreen extends StatefulWidget {
  final ToolViewModel viewModel;
  final ToolEntity? tool; // null = crear | ToolEntity = editar

  const ToolFormScreen({super.key, required this.viewModel, this.tool});
...
class _ToolFormScreenState extends State<ToolFormScreen> {
  ...
  Future<void> _save() async {
    ...
    bool ok;
    if (_isEditing) {
      ok = await widget.viewModel.updateTool(
        id:          widget.tool!.id,
      ...
      ok = await widget.viewModel.createTool(
        name:        _nameCtrl.text.trim(),
        description: _descCtrl.text.trim(),
        category:    _catCtrl.text.trim(),
        isAvailable: _isAvailable,
    ...
    if (ok) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(widget.viewModel.error ?? 'Error al guardar'),
        ...
        child: AnimatedBuilder(
          ...
          builder: (context, _) => Form(
            ...
            child: ListView(
              ...
              children: [
                // Stack: imagen + botón cámara superpuesto
                Center(
                  child: Stack(
                ...
                FilledButton.icon(
                  onPressed: widget.viewModel.loading ? null : _save,
                  icon: widget.viewModel.loading
                      ? SizedBox(
                          height: 18, width: 18,
